...
| `EBAY_MARKETPLACE_ID` / `EBAY_CATEGORY_TREE_ID` / `EBAY_CURRENCY` | optional, experimental | Marketplace override, e.g. `EBAY_GB` / `3` / `GBP` for eBay UK — set all three together. Defaults: `EBAY_US` / `0` / `USD`. ⚠️ **The US site is the only tested marketplace.** Known gaps on other sites: photo uploads still use the US site ID, and condition-tier / size-standardization handling were validated against eBay US. After changing marketplace, regenerate the offline category map: `npx tsx scripts/refresh-category-map.ts`. The listing cards show **$** by default; use **⚙ Settings → Price currency** to switch £ / € / A$ / C$ (and to have the AI price in that currency). Posting still uses `EBAY_CURRENCY`. |
