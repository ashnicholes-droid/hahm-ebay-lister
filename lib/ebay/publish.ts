...
import { createSellerHubDraft } from "./draft-feed";
...
// 3. Offer.
  ...
  if (!Number.isFinite(price) || price <= 0) {
    return {
      success: false,
      sku,
      error: `Invalid eBay price: ${listing.suggested_price}`,
    };
  }
  ...
  // 4. Draft only — do NOT publish live.
  console.log("[eBay draft saved]", { sku, offerId, price: offerBody.pricingSummary.price.value });
  return {
    success: true,
    sku,
    offerId,
...
// Draft only — do NOT publish live.
// Offer has been created and left in DRAFT state.
return {
  success: true,
  sku,
  offerId,
